...
def generate_copy_seed_sequence(seed_length, training_data):
	...
	seedSeq = np.reshape(randSeed, (seed_length, example_len, randSeed.shape[1]))
	return seedSeq

def generate_random_seed_sequence(seed_length, training_data):
	num_examples = training_data.shape[0]
	example_len = training_data.shape[1]
	seedSeq = np.random.rand(1, 1, 11024)
	return seedSeq
